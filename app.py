            ...
            raise AuthError(
                {
                    ...
                    f"Use an {app.config['ALGORITHMS']} signed JWT Access Token",
                },
                401,
            ) from jwt_error
        ...
        if rsa_key:
            try:
                ...
                raise AuthError(
                    {
                        ...
                        f"Issuer : https://{app.config['AUTH0_DOMAIN']}/",
                    },
                    401,
                ...
                raise AuthError(
                    {
                        "code": "invalid_header",
                        "description": "Unable to parse authentificationtoken.",
                    },
                    401,
                ) from exc
...
command_service = Enum(
    "command_service", ["command_id", "command_user_name", "service_name", "quantity"]
)
...
def Operate(model: str):
    ...
    # Get primary Key
    cursor.execute(
        f"SHOW KEYS FROM {app.config['MYSQL_DB']}.{model} WHERE Key_name = 'PRIMARY'"
    )
    ...
    if request.method == "POST":
        ...
        cursor = mysql.new_cursor(dictionary=True, buffered=True)
        create = ", ".join(
            [f"`{att}`" for att in request.json.keys()]
        )  # Attributes here needs to be embeded in `` because they are reserved words in SQL
    ...
    if request.method == "DELETE":
        ...
        cursor = mysql.new_cursor(dictionary=True, buffered=True)
        cursor.execute(
            f"DELETE FROM {app.config['MYSQL_DB']}.{model} WHERE {primary_key} = {FormatTextToMysql(request.json[primary_key], 1)}"
        )
        mysql.connection.commit()
        return "True"
    if request.method == "PUT":
        ...
        if model == "params":
            updates = ", ".join(
                [
                    f"{att} = {request.json[att]}"
                    for att in request.json.keys()
                    if att != primary_key
                ]
            )
            ...
            updates = ", ".join(
                [
                    f"{att} = {FormatTextToMysql(request.json[att], 1)}"
                    for att in request.json.keys()
                    if att != primary_key
                ]
            )
...
def GetAvailableHours(date):
    ...
    cur = mysql.new_cursor(dictionary=True)
    cur.execute(
        f"SELECT enter_time,return_time FROM {app.config['MYSQL_DB']}.command WHERE enter_date = '{date}'"
    )
    a = cur.fetchall()
...
def pricing(lang="fr", subpage=None, store=None):
    ...
    categories = sorted(
        GetData("full", "category"), key=lambda x: x["index"]
    )  # Get just the categories
    services_in_store = GetData(
        "full", "store_has_service"
    )  # Get all services associates to all stores
    ...
    selected_category = (
        [x for x in categories if x["name"] == subpage][0] if subpage else None
    )
    ...
    return render_template(
        ...
        lang=session["lang"],
        categories=categories,
        services_in_store=services_in_store,
        dictionary=dictionary[lang],
        category=subpage,
        other_categories=others_categories,
        services=None if len(available_services) <= 0 else available_services,
...
def localization(lang="fr", subpage=None):
    ...
    available_store = (
        [x for x in stores if x["name"] == subpage][-1] if subpage else None
    )
    ...
    return render_template(
        ...
        dictionary=dictionary[lang],
        stores=stores,
        selected_store=available_store,
        current_page="localization",
        services=services,
        categories=categories,
    )
...
def GetDictionary():
    ...
    # Get the babel .po
    po = polib.pofile(
        f"{app.config['BABEL_PO_FILES_PATH']}/{lang}/LC_MESSAGES/messages.po"
    )
...
def updatewebsitedictionnary():
    ...
    subprocess.run(
        ["pybabel", "compile", "-d", app.config["BABEL_PO_FILES_PATH"]], check=True
    )
...
def updateBabel():
    ...
    updating_process = subprocess.Popen(
...
def UpdateParams():
    if request.method == "POST":
        ...
        cursor.execute(
            f"UPDATE `appexpress`.`params` SET `tarif_bruxelles` = '{tarif_bruxelles}', `tarif_brabant` = '{tarif_brabant}', `tarif_km` = '{tarif_km}' WHERE (`id` = '1');"
        )
        mysql.connection.commit()
        return "True"
...
def secureConnection():
    if request.method == "POST":
        ...
        if username != "":
            cur.execute(
                f"SELECT * FROM {app.config['MYSQL_DB']}.user WHERE name = '{username}' and password = '{password}'"
            )
        elif email != "":
            cur.execute(
                f"SELECT * FROM {app.config['MYSQL_DB']}.user WHERE mail = '{email}' and password = '{password}'"
            )
        a = cur.fetchone()
        print(a)
        return json.dumps(a)
...
def check_user():
    if request.method == "POST":
        ...
        if username != "":
            ...
            cur.execute(
                f"SELECT password FROM appexpress.user WHERE name = '{username}'"
            )
        elif email != "":
...
def UpdateUser():
    ...
    cursor = mysql.new_cursor(dictionary=True)
    cursor.execute(
        f"UPDATE `appexpress`.`user` SET `name` = '{username}', `mail` = '{mail}', `adress` = '{adress}', `phone` = '{phone}', `password` = '{password}', `type` = '{account}', `loc_lat` = '{loc_lat}', `loc_lon` = '{loc_lon}', `province` = '{province}'  WHERE (`id` = '{id_}');"
    )
    mysql.connection.commit()
...
def AddUser():
    if request.method == "POST":
        ...
        province = request.json["province"]
        cur.execute(
            f"INSERT INTO `appexpress`.`user` (`name`, `mail`, `adress`, `phone`, `password`, `type`, `loc_lat`, `loc_lon`, `province`) VALUES ('{nameUser}', '{email}', '{addressUser}', '{phoneUser}', '{passwordUser}', '{typeUser}', '{loc_lat}', '{loc_lon}', '{province}')"
        )
        mysql.connection.commit()
        id = cur.lastrowid
...
def AddCommand():
    if request.method == "POST":
        ...
        cur = mysql.new_cursor(dictionary=True)
        cur.execute(
            f"INSERT INTO `appexpress`.`command` (`id`, `infos`, `cost`, `enter_date`, `return_date`,`discount`, `services_quantity`, `user`, `enter_time`, `return_time`, `sub_total`, `delivery`) VALUES ('{id}', '{status}', '{cost}', '{enter_date}', '{return_date}', '{discount}', '{services_quantity}', '{user}', '{enter_time}', '{return_time}','{sub_total}','{delivery}')"
        )
        ...
        for i in range(len(services_quantity.split(","))):
            cur.execute(
                f"INSERT INTO `appexpress`.`command_service` (`command_id`, `service_id`, `quantity`) VALUES ('{id}', '{services_quantity.split(',')[i].split(':')[0]}', '{services_quantity.split(',')[i].split(':')[1]}')"
            )
            mysql.connection.commit()
...
def AddServiceHasCommand():
    if request.method == "GET":
        ...
        cur = mysql.new_cursor()
        cur.execute(
            "INSERT INTO `appexpress`.`service_has_command` (`service_id`, `command_id`, `quantity`) VALUES ('"
            + str(service)
            + "', '"
            + str(commandid)
            + "', '"
...
def AddCommandToUser():
    ...
    cursor = mysql.new_cursor(dictionary=True)
    cursor.execute(
        "INSERT INTO `appexpress`.`command_has_user` (`command_id`, `user`) VALUES ('"
        + str(command)
        + "', '"
        + str(user)
        + "')"
...
def AddCoupon():
    if request.method == "POST":
        ...
        cursor = mysql.new_cursor()
        cursor.execute(
            f"INSERT INTO `appexpress`.`coupon` (`code`, `discount`) VALUES ('{codeCoupon}', '{cost}')"
        )
        mysql.connection.commit()
        return "True"
...
def DeleteUser():
    if request.method == "POST":
        ...
        cursor = mysql.new_cursor(dictionary=True)
        cursor.execute(
            "DELETE FROM `appexpress`.`user` WHERE (`id` = '" + str(user) + "')"
        )
        mysql.connection.commit()
...
def DeleteService():
    if request.method == "POST":
        ...
        cursor = mysql.new_cursor(dictionary=True)
        cursor.execute(
            "DELETE FROM `appexpress`.`service` WHERE (`id` = '" + str(service) + "')"
        )
        mysql.connection.commit()
    return "True"
...
def SecureConnection():
    if request.method == "POST":
        ...
        cursor = mysql.new_cursor(dictionary=True)
        cursor.execute(
            "SELECT password FROM appexpress.user WHERE name = '" + username + "'"
        )
...
def UpdateCommand():
    ...
    cursor = mysql.new_cursor()
    cursor.execute(
        f"UPDATE `appexpress`.`command` SET `cost` = '{cost}', `enter_date` = '{dateIn}', `return_date` = '{dateOut}', `services_quantity` = '{services_quantity}', `agent` = '{agent}', `enter_time` = '{enter_time}', `return_time` = '{return_time}', `sub_total` = '{sub_total}', `delivery` = '{delivery}' WHERE (`id` = '{Id}');"
    )
    mysql.connection.commit()
    return "True"
...
def DeleteCommand():
    if request.method == "POST":
        ...
        cursor = mysql.new_cursor(dictionary=True)
        cursor.execute(
            "DELETE FROM `appexpress`.`command` WHERE (`id` = '" + str(command) + "')"
        )
        mysql.connection.commit()
    return "True"
...
def fill_database():
    ...
    for service in services:
        for store in stores:
            cursor.reset()
            cursor.execute(
                f"INSERT INTO `{app.config['MYSQL_DB']}`.`store_has_service` (`store_name`, `service_name`,`cost`) VALUES ('{store['name']}', '{service['name']}', '5.00');"
            )
            mysql.connection.commit()
...
dictionary = {
    "fr": {
        ...
        "message": "Message",
        "about-text1": [
            "Vous recherchez une blanchisserie non loin d'Ixelles et Bruxelles ? ",
            "Vous avez besoin d'un service de pressing à domicile ? ",
            "Vous souhaitez faire nettoyer votre linge de maison par des professionnels ? ",
            "Découvrez les services de notre pressing écologique. ",
        ],
        "about-text2": [
            "Ex-Press Dry Clean,",
            "c’est plus de 20 ans d’expérience auprès de nos clients particuliers et professionnels à Bruxelles et dans toutes les communes environnantes. ",
            "Nos experts du linge prennent soin de vos vêtements, de votre linge de maison ainsi que de vos chaussures au quotidien. Service de blanchisserie ou nettoyage à sec pour les textiles les plus fragiles, nous nous adaptons à vos exigences et celles de votre linge.",
            "du secteur, nous sommes à l’affût des dernières nouveautés en termes de pressing écologique, technologies et produits professionnels mis en vente sur le marché pour prendre soin de votre linge.  Nous les renouvelons régulièrement afin de vous assurer un service parfait en toutes circonstances.",
        ],
        "about-text3": [
            "Notre pressing écologique vous propose un service de qualité pour nettoyer vos vêtements, votre linge de maison et vos chaussures. ",
            "Nous utilisons des produits de nettoyage respectueux de l’environnement pour prendre soin de votre linge et de vos vêtements. ",
            "Nous vous proposons un service de pressing à domicile pour vous faciliter la vie. ",
            "Nous collectons, nettoyons et livrons vos vêtements à domicile ou au bureau selon vos disponibilités. ",
            "Depuis de nombreuses années, nous ne travaillons plus avec le Perchloro-éthylène. Ce solvant autrefois utilisé par les blanchisseries industrielles pour le nettoyage à sec a été classé parmi les substances cancérigènes, hautement toxiques et polluantes par les Hautes Autorités Européennes. Nous l’avons remplacé par le HiGlo pour ce qui est de notre machine situé à Etterbeek et nous utilisons le Sensene pour notre centrale de Zaventem. Des Solvants tout aussi efficace  et entièrement écologique et biodégradable. Pour enlever vos taches, nous utilisons désormais des produits 100% écologiques respectueux de votre santé et de la planète. L’écologie, à l’instar de vos taches, est aussi notre cheval de bataille !",
        ],
        "about-text4": [
            "Nous sommes fiers de vous proposer des services professionnels de blanchisserie, de nettoyage à sec et de couture à Etterbeek, St-Josse, Moortebeek et Zaventem. Trois adresses différentes sous une même enseigne pour être chaque jour un peu plus près de nos clients !"
        ],
        "strengths": [
            {
                "title": "work1_title",
    ...
    "en": {
        ...
        "message": "Message",
        "about-text1": [
            "Are you looking for a laundry not far from Ixelles and Brussels? ",
            "Do you need a home dry cleaning service? ",
            "Do you want to have your household linen cleaned by professionals? ",
            "Discover the services of our ecological dry cleaning. ",
        ],
        "about-text2": [
            "Ex-Press Dry Clean,",
            "it is more than 20 years of experience with our private and professional clients in Brussels and all surrounding municipalities. ",
            "Our linen experts take care of your clothes, your household linen as well as your shoes on a daily basis. Laundry service or dry cleaning for the most fragile textiles, we adapt to your requirements and those of your linen.",
            "sector, we are on the lookout for the latest news in terms of ecological dry cleaning, technologies and professional products put on sale on the market to take care of your linen. We renew them regularly to ensure you a perfect service in all circumstances.",
        ],
        "about-text3": [
            "Our ecological dry cleaning offers you a quality service to clean your clothes, household linen and shoes. ",
            "We use environmentally friendly cleaning products to take care of your linen and clothes. ",
            "We offer you a home dry cleaning service to make your life easier. ",
            "We collect, clean and deliver your clothes to your home or office according to your availability. ",
            "For many years, we no longer work with Perchloroethylene. This solvent formerly used by industrial laundries for dry cleaning has been classified as carcinogenic, highly toxic and polluting by the European High Authorities. We have replaced it with HiGlo for our machine located in Etterbeek and we use Sensene for our Zaventem plant. Solvents just as effective and entirely ecological and biodegradable. To remove your stains, we now use 100% ecological products that are respectful of your health and the planet. Ecology, like your stains, is also our battle horse!",
        ],
        "about-text4": [
            "We are proud to offer you professional laundry, dry cleaning and sewing services in Etterbeek, St-Josse, Moortebeek and Zaventem. Three different addresses under the same sign to be a little closer to our customers every day!"
        ],
        "strengths": [
            {
    ...
    "nl": {
        ...
        "message": "Bericht",
        "about-text1": [
            "Bent u op zoek naar een wasserij niet ver van Elsene en Brussel? ",
            "Heeft u een thuisdroogreinigingsservice nodig? ",
            "Wilt u uw huishoudlinnen laten reinigen door professionals? ",
            "Ontdek de diensten van onze ecologische droogkuis. ",
        ],
        "about-text2": [
            "Ex-Press Dry Clean,",
            "is meer dan 20 jaar ervaring bij onze particuliere en professionele klanten in Brussel en alle omliggende gemeenten. ",
            "Onze linnenexperts zorgen dagelijks voor uw kleding, uw huishoudlinnen en uw schoenen. Wasserij- of droogreinigingsdienst voor de meest kwetsbare textielen, wij passen ons aan uw eisen en die van uw linnen aan.",
            "sector, zijn we op de hoogte van het laatste nieuws op het gebied van ecologische droogreiniging, technologieën en professionele producten die op de markt worden verkocht om voor uw linnen te zorgen. We vernieuwen ze regelmatig om u een perfecte service in alle omstandigheden te garanderen.",
        ],
        "about-text3": [
            "Onze ecologische droogkuis biedt u een kwaliteitsvolle service om uw kleding, huishoudlinnen en schoenen te reinigen. ",
            "We gebruiken milieuvriendelijke reinigingsproducten om voor uw linnen en kleding te zorgen. ",
            "We bieden u een thuisdroogreinigingsservice om uw leven gemakkelijker te maken. ",
            "Wij halen, reinigen en leveren uw kleding aan huis of op kantoor volgens uw beschikbaarheid. ",
        ...
        "about-text4": [
            "We zijn er trots op u professionele wasserij-, droogreinigings- en naaiservices aan te bieden in Etterbeek, St-Josse, Moortebeek en Zaventem. Drie verschillende adressen onder hetzelfde bord om elke dag een beetje dichter bij onze klanten te zijn!"
        ],
        "strengths": [
            {
    ...
    "it": {
        ...
        "message": "Messaggio",
        "about-text1": [
            "Stai cercando una lavanderia non lontano da Ixelles e Bruxelles? ",
            "Hai bisogno di un servizio di lavaggio a secco a domicilio? ",
            "Vuoi far pulire il tuo bucato da professionisti? ",
            "Scopri i servizi del nostro lavaggio a secco ecologico. ",
        ],
        "about-text2": [
            "Ex-Press Dry Clean,",
            "è più di 20 anni di esperienza con i nostri clienti privati e professionali a Bruxelles e in tutti i comuni circostanti. ",
            "I nostri esperti di biancheria si prendono cura dei tuoi vestiti, della tua biancheria da casa e delle tue scarpe quotidianamente. Servizio di lavanderia o lavaggio a secco per i tessuti più fragili, ci adattiamo alle tue esigenze e a quelle della tua biancheria.",
            "settore, siamo sempre alla ricerca delle ultime novità in materia di lavaggio a secco ecologico, tecnologie e prodotti professionali messi in vendita sul mercato per prendersi cura della tua biancheria. Li rinnoviamo regolarmente per garantirti un servizio perfetto in tutte le circostanze.",
        ],
        "about-text3": [
            "Il nostro lavaggio a secco ecologico ti offre un servizio di qualità per pulire i tuoi vestiti, la tua biancheria da casa e le tue scarpe. ",
            "Utilizziamo prodotti per la pulizia rispettosi dell'ambiente per prenderti cura della tua biancheria e dei tuoi vestiti. ",
            "Ti offriamo un servizio di lavaggio a secco a domicilio per semplificarti la vita. ",
            "Ritiriamo, puliamo e consegniamo i tuoi vestiti a casa o in ufficio secondo la tua disponibilità. ",
            "Da molti anni non lavoriamo più con il Percloroetilene. Questo solvente usato in passato dalle lavanderie industriali per il lavaggio a secco è stato classificato come cancerogeno, altamente tossico e inquinante dalle Autorità Europee. L'abbiamo sostituito con l'HiGlo per la nostra macchina situata a Etterbeek e utilizziamo il Sensene per la nostra centrale di Zaventem. Solventi altrettanto efficaci e completamente ecologici e biodegradabili. Per rimuovere le macchie, ora utilizziamo prodotti al 100% ecologici rispettosi della tua salute e del pianeta. L'ecologia, come le tue macchie, è anche il nostro cavallo di battaglia!",
        ],
        "about-text4": [
            "Siamo orgogliosi di offrirti servizi professionali di lavanderia, lavaggio a secco e sartoria a Etterbeek, St-Josse, Moortebeek e Zaventem. Tre indirizzi diversi sotto lo stesso segno per essere un po' più vicini ai nostri clienti ogni giorno!"
        ],
        "strengths": [
            {
