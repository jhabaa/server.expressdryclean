...
app.config['MYSQL_USER'] = 'Database_name'
...
def chatwithus():
    ...
    print(data)
    #send mail to admin to inform him about the message
    send_email(to_addr="***REMOVED***", subject="Message de "+data['first_name'] + data['last_name'], user_name=data['first_name'], email_type="contact_toadmin", command_recap=data['subject'])
...
def fill_database():
    cursor = mysql.new_cursor( dictionary = True)
    # get stores
    cursor.execute("SELECT * FROM Database_name.store")
    stores = cursor.fetchall()
    cursor.reset()
    # get services
    cursor.execute("SELECT * FROM Database_name.service")
    services = cursor.fetchall()
    #fill the store_has_service table
    for service in services:
        for store in stores:
            cursor.reset()
            print((f"INSERT INTO `Database_name`.`store_has_service` (`store_name`, `service_name`,`cost`) VALUES ('{store['name']}', '{service['name']}', '5.00');"))
            cursor.execute(f"INSERT INTO `Database_name`.`store_has_service` (`store_name`, `service_name`,`cost`) VALUES ('{store['name']}', '{service['name']}', '5.00');")
            mysql.connection.commit()
